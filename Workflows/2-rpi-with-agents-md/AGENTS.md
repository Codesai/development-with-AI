...
3. **Implement**: Make only the changes required for the approved feature. Preserve valid behavior and add focused automated tests. All validations should be run with `make validate`.
4. **Review**: Ask a separate subagent to review the changes. Report findings under `HIGH`, `MEDIUM`, and `LOW` severity. 
