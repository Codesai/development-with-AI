...
work with subagents in copilot cli
...
inside the folder /ContextManagement (start copilot from this folder):

define two subagents:
    - one to work with the frontend code
        - for example create the file -> .github/agents/front-dev.agend.md
    - the second to work with the backend code
        - for example create the file -> .github/agents/back-dev.agend.md
...
- copilot infers automatically if there is a custom defined agent suitable for the task and delegates the task in any normal prompt (but sometimes decide to not delegate... for example for
small tasks)
...
- with the command "/fleet" you can send a prompt with an implementation plan and copilot divide
this plan in subtask than can be delegated to subagents and executed in parallel. if you want a subtask to use a specfic agent use @agent-name, for example "/fleet validate the email, use @frontend-dev for the fornt validations and @backend-dev for the validations in the back"
